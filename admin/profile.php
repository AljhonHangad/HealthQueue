    ...
    if (!verifyCsrfToken($_POST['csrf_token'] ?? null)) {
        ...
        if (!$errors) {
            try {
                ...
                if ($dupe->fetch()) {
                    ...
                    logActivity($pdo, $user['UserID'], null, 'Updated profile', 'Admin updated their own account details.');
        ...
        try {
            ...
            if (!$row || !password_verify($current, $row['PasswordHash'])) {
                ...
                logActivity($pdo, $user['UserID'], null, 'Changed password', 'Admin changed their own password.');
...
<main class="admin-shell"><div class="container">
  <section class="admin-hero"><div><span class="eyebrow">Account settings</span><h1>My profile</h1><p>Update your own account details and password.</p></div></section>
