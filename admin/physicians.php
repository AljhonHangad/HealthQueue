    ...
    if (!verifyCsrfToken($_POST['csrf_token'] ?? null)) {
        ...
        if (!$errors) {
            try {
                ...
                if (!$clinicStmt->fetch()) {
                    ...
                    logActivity($pdo, $user['UserID'], (int) $values['clinic_id'], 'Updated physician', $values['first_name'] . ' ' . $values['last_name']);
                    $flash = 'Physician account updated.';
                } else {
                ...
                if (!$errors) {
                    header('Location: ' . HQ_BASE_URL . '/admin/physicians.php');
                    exit;
                }
            } catch (PDOException $e) {
                error_log('Physician save failed: ' . $e->getMessage());
        ...
        if (!$memberId || !in_array($status, ['Active', 'Inactive'], true)) {
            ...
            try {
                ...
                if ($stmt->rowCount()) {
                    logActivity($pdo, $user['UserID'], null, 'Changed physician status', "UserID {$memberId} set to {$status}.");
                    $flash = 'Account status updated.';
                } else {
                    $errors[] = 'The account could not be found.';
                }
            } catch (PDOException $e) {
                error_log('Physician status update failed: ' . $e->getMessage());
...
if ($pdo) {
    try {
        ...
        error_log('Physicians list failed: ' . $e->getMessage());
        $errors[] = 'Physician information is temporarily unavailable.';
    }
}

$pageTitle = 'Manage Physicians — HealthQueue';
require __DIR__ . '/../includes/header.php';
?>

<main class="admin-shell"><div class="container">
  <section class="admin-hero"><div><span class="eyebrow">Team management</span><h1>Physicians</h1><p>Create physician accounts, assign them to a clinic, and manage their status.</p></div></section>
  ...
  <section class="admin-section" id="member-form">
    <div class="portal-heading"><div><span class="section-kicker"><?= $editingMember ? 'Edit physician' : 'Add physician' ?></span><h2><?= $editingMember ? htmlspecialchars($editingMember['FirstName'] . ' ' . $editingMember['LastName']) : 'New physician account' ?></h2></div></div>
    <form class="registration-form" method="post" style="max-width:640px;">
      ...
      <?php if ($editingMember): ?><a href="<?= HQ_BASE_URL ?>/admin/physicians.php" class="text-link" style="display:block;text-align:center;margin-top:12px;">Cancel edit</a><?php endif; ?>
    </form>
  </section>

  <section class="admin-section">
    <div class="portal-heading"><div><span class="section-kicker">Directory</span><h2>All physicians</h2></div></div>
    <form method="get" class="admin-search">
      ...
      <?php if ($search !== ''): ?><a href="<?= HQ_BASE_URL ?>/admin/physicians.php" class="text-link">Clear</a><?php endif; ?>
    </form>
    <div class="admin-table-wrap">
      <table class="admin-table">
        ...
        <?php foreach ($members as $member): ?>
          <tr>
            <td><strong>Dr. <?= htmlspecialchars($member['FirstName'] . ' ' . $member['LastName']) ?></strong></td>
            ...
            <td>
              <div style="display:flex;gap:6px;flex-wrap:wrap;">
                <a href="<?= HQ_BASE_URL ?>/admin/physicians.php?edit=<?= (int) $member['UserID'] ?>#member-form" class="btn btn-outline btn-sm">Edit</a>
                <form method="post"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars(csrfToken()) ?>"><input type="hidden" name="form_type" value="update_status"><input type="hidden" name="member_id" value="<?= (int) $member['UserID'] ?>">
                  <?php if ($member['Status'] === 'Active'): ?><button class="btn btn-outline btn-sm" name="status" value="Inactive" onclick="return confirm('Deactivate this account? The physician will no longer be able to sign in.');">Deactivate</button>
        ...
        <?php endforeach; ?>
        <?php if (!$members): ?><tr><td colspan="6" class="admin-empty"><?= $search !== '' ? 'No physicians match "' . htmlspecialchars($search) . '".' : 'No physician accounts yet.' ?></td></tr><?php endif; ?>
        </tbody>
      </table>
    </div>
  </section>
