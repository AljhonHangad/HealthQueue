...
if ($pdo) {
    try {
        ...
        error_log('Physician profile load failed: ' . $e->getMessage());
    }
}
    ...
    if (!verifyCsrfToken($_POST['csrf_token'] ?? null)) {
            ...
            try {
                ...
                if ($dupe->fetch()) {
                    ...
                    $pdo->prepare('UPDATE Users SET FirstName = ?, LastName = ?, Email = ?, ContactNumber = ? WHERE UserID = ?')
                        ->execute([$firstName, $lastName, $email, $contact, $user['UserID']]);
                    ...
                    logActivity($pdo, $user['UserID'], $user['ClinicID'], 'Updated profile', 'Physician updated their own account details.');
                ...
                error_log('Physician profile update failed: ' . $e->getMessage());
        ...
        try {
            ...
            if (!$row || !password_verify($current, $row['PasswordHash'])) {
                ...
                logActivity($pdo, $user['UserID'], $user['ClinicID'], 'Changed password', 'Physician changed their own password.');
                $flash = 'Password changed.';
            }
        } catch (PDOException $e) {
            error_log('Physician password change failed: ' . $e->getMessage());
...
<main class="portal-shell"><div class="container">
  <section class="portal-hero"><div><span class="eyebrow">Account settings</span><h1>My profile</h1><p>Update your own account details and password.</p></div></section>
  ...
  <section class="portal-section">
    ...
    <form class="registration-form" method="post" style="max-width:560px;">
      ...
      <div class="form-stack">
        ...
        <label>Contact number<input type="tel" name="contact_number" value="<?= htmlspecialchars($profile['ContactNumber'] ?? '') ?>" required></label>
      </div>
